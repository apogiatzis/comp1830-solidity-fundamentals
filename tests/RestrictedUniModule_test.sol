...
import "../4-modifiers/RestrictedUniModule.sol";
...
contract testSuite {
    ...
    RestrictedUniModule uniModule;
    uint256 price = 100;

    function beforeAll() public {
        uniModule = new RestrictedUniModule(price);
    }
    ...
    /// #value: 100
    function checkSetModuleRepByLeader() public payable{
    ...
    /// #value: 100
    /// #sender: account-2
    function checkSetModuleRepByNonLeader() public payable{
        uniModule.register{value: msg.value}("Georgios", "0000001");
        try uniModule.setModuleRep(address(this)) returns (bool) {} catch  {
            Assert.ok(true, "setModuleRep should fail if set by other than module leader");
        } 
    }
